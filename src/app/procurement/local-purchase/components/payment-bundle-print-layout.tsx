...
// Disable SSR for the component that uses pdfjs-dist
const PdfMultiPageRenderer = dynamic(() => import('./pdf-multi-page-renderer'), { ssr: false });
...
const UploadedDocumentPage: React.FC<{ file: { name: string; file: string }; label: string; orgSettings: OrganizationSettings }> = ({ file, label, orgSettings }) => {
    ...
    // High-Fidelity Branching: Multi-page PDFs are decomposed into individual sheets via client-only renderer
    if (isPdf) {
