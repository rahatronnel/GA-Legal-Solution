 ...
 * Returns: { text: "3 days, 4 hours and 14 minutes ago", color: "text-red-500" }
 */
export function getRelativeTimeInfo(timestamp: string) {
    ...
    if (diffDays > 0) {
        text = `${diffDays} day${diffDays > 1 ? 's' : ''}, ${hours} hour${hours !== 1 ? 's' : ''} and ${minutes} minute${minutes !== 1 ? 's' : ''} ago`;
        if (diffDays >= 3) color = 'text-destructive font-bold';
        else color = 'text-orange-500 font-semibold';
    } else if (hours > 0) {
        text = `${hours} hour${hours > 1 ? 's' : ''} and ${minutes} minute${minutes !== 1 ? 's' : ''} ago`;
