...
export async function recreateUserWithPassword(auth: Auth, email: string, newPassword: string): Promise<void> {
  ...
  // This is a placeholder for the superadmin password.
  const superadminPassword = superadminEmail === 'superadmin@galsolution.com' ? "superadmin@#%" : "sysadmin2026";
  ...
  // 4. Sign back in as the admin again to restore the session state
  await signInWithEmailAndPassword(auth, superadminEmail, superadminPassword);
}